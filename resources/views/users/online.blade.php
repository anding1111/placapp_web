...
class OnlineUsersList {
    ...
    // Método para obtener el token CSRF dinámicamente
    getCsrfToken() {
    ...
    async fetchUsers() {
        ...
        try {
            ...
            if (this.active) this.renderUsers(users);
        } catch (error) { 
            console.error('Error fetching online users:', error); 
        } finally {
            this.isUpdating = false;
            if (this.active) {
                this.timeoutId = setTimeout(() => this.fetchUsers(), this.interval);
            }
        }
    }
                ...
                <div class="ios-user-card">
                    ...
                    <div class="ios-user-info">
                        ...
                        <div class="ios-user-role">
                            ...
                            <span>${roleText} • @${this.escapeHtml(user.user || 'usuario')}</span>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    // Solo iniciar la lista si el contenedor de perfiles existe (evita fugas en otras vistas)
    if (document.querySelector('#leaderboard__profiles')) {
